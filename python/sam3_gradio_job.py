...
_JOB_PROPAGATE_CHUNK_MAX_FRAMES = 256
_JOB_PROPAGATE_CHUNK_OVERLAP_FRAMES = 32
...
def _select_device(pref: str) -> str:
    ...
    if p.startswith("cpu"):
        raise RuntimeError("CPU execution is disabled. Set options.device_preference to 'cuda'.")
    if not (p.startswith("cuda") or p in ("gpu", "auto", "")):
        raise RuntimeError(f"Unsupported options.device_preference: {pref!r}. Only 'cuda' is allowed.")
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA GPU is required, but no CUDA device is available.")
    return "cuda"

def _normalize_torch_device_name(value: str, fallback: str = "cuda") -> str:
...
def _default_session_devices(job_device: str) -> Tuple[str, str, str]:
    d = _normalize_torch_device_name(job_device, "cuda")
    if d != "cuda":
        raise RuntimeError("CPU fallback is disabled. Session devices must be CUDA.")
...
def _set_session_devices(processing_device: str, video_storage_device: str, inference_state_device: str) -> None:
    ...
    pdev = _normalize_torch_device_name(processing_device, "cuda")
    vdev = _normalize_torch_device_name(video_storage_device, "cuda")
    sdev = _normalize_torch_device_name(inference_state_device, "cuda")
    if pdev != "cuda" or vdev != "cuda" or sdev != "cuda":
        raise RuntimeError("CPU fallback is disabled. session_devices must be all 'cuda'.")
    _JOB_SESSION_PROCESSING_DEVICE = "cuda"
    _JOB_SESSION_VIDEO_STORAGE_DEVICE = "cuda"
    _JOB_SESSION_INFERENCE_STATE_DEVICE = "cuda"
...
def _session_device_profiles() -> List[Tuple[str, str, str]]:
    ...
    if base != ("cuda", "cuda", "cuda"):
        raise RuntimeError("CPU fallback is disabled. Session profile must be CUDA only.")
    return [base]
...
def _apply_request_defaults(req: dict) -> None:
    ...
    global _JOB_SESSION_CHUNK_FRAMES, _JOB_PROPAGATE_CHUNK_MAX_FRAMES, _JOB_PROPAGATE_CHUNK_OVERLAP_FRAMES, _JOB_FRAME_MEM_BUDGET_RATIO, _JOB_FRAME_MAX_COUNT, _JOB_ONDEMAND_CACHE_MAX_FRAMES, _JOB_UI_WAIT_TIMEOUT_SEC
    ...
    raw_prop_chunk = opts.get("propagate_chunk_max_frames", opts.get("propagate_chunk_frames", 256))
    try:
        prop_chunk = int(raw_prop_chunk)
    except Exception:
        prop_chunk = 256
    _JOB_PROPAGATE_CHUNK_MAX_FRAMES = min(4096, max(32, prop_chunk))

    raw_prop_overlap = opts.get("propagate_chunk_overlap_frames", opts.get("propagate_overlap_frames", 32))
    try:
        prop_overlap = int(raw_prop_overlap)
    except Exception:
        prop_overlap = 32
    prop_overlap = min(1024, max(0, prop_overlap))
    if prop_overlap >= _JOB_PROPAGATE_CHUNK_MAX_FRAMES:
        prop_overlap = max(0, _JOB_PROPAGATE_CHUNK_MAX_FRAMES - 1)
    _JOB_PROPAGATE_CHUNK_OVERLAP_FRAMES = int(prop_overlap)
...
def _ensure_cuda_triton_ready() -> None:
    global _JOB_USE_TORCH_COMPILE
    if _JOB_DEVICE != "cuda":
        raise RuntimeError("CPU fallback is disabled. options.device_preference must be 'cuda'.")
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA GPU is required, but no CUDA device is available.")
...
def _with_ffmpeg_quiet_globals(cmd: List[str]) -> List[str]:
    if not cmd:
        return cmd
    exe = cmd[0]
    rest = list(cmd[1:])
    # Keep ffmpeg logs minimal to avoid huge stderr accumulation on long clips.
    quiet_globals = ["-hide_banner", "-loglevel", "error", "-nostats", "-nostdin"]
    out = [exe]
    for tok in quiet_globals:
        if tok not in rest:
            out.append(tok)
    out.extend(rest)
    return out
...
def _run_ffmpeg(cmd: List[str]) -> Tuple[int, str]:
    # Stream stderr and keep only a bounded tail so ffmpeg logs do not exhaust RAM.
    run_cmd = _with_ffmpeg_quiet_globals(cmd)
    ...
    try:
        while True:
            ...
            if len(stderr_tail) > max_tail_bytes:
                del stderr_tail[:-max_tail_bytes]
    finally:
        try:
            p.stderr.close()
        except Exception:
            pass
    code = p.wait()
    return code, _decode_subprocess_text(bytes(stderr_tail))
...
def _propagate_masks_full_session_legacy(
    ...
    chunk_size = max(32, int(_JOB_PROPAGATE_CHUNK_MAX_FRAMES))
    overlap = max(0, int(_JOB_PROPAGATE_CHUNK_OVERLAP_FRAMES))
    if overlap >= chunk_size:
        overlap = max(0, chunk_size - 1)
    decode_window = int(chunk_size + overlap)
    ...
    state.current_frame_idx = int(anchor_frame_idx)
    ...
    def _load_window_session(window_start: int, window_end_target: int) -> Tuple[int, int]:
        total_frames = int(state.num_frames)
        ws = int(np.clip(int(window_start), 0, max(0, total_frames - 1)))
        we_target = int(np.clip(int(window_end_target), ws + 1, total_frames))
        target_len = max(1, int(we_target) - int(ws))
        min_decode = max(32, min(target_len, max(1, int(overlap) + 1)))
        chunk_candidates = _chunk_retry_candidates(target_len, min_chunk=min_decode)
        last_err: Optional[Exception] = None

        for i, cand_len in enumerate(chunk_candidates):
            we = min(int(we_target), int(ws) + int(cand_len))
            if we <= ws:
                continue
            _release_loaded_session()
            try:
                state.inference_session = _build_streaming_inference_session(processor)
                _load_frame_chunk_into_session(state, processor, int(ws), int(we))
                state.loaded_chunk_start_idx = int(ws)
                state.loaded_chunk_end_idx = int(we)
                _replay_prompts_for_chunk(state, processor, int(ws), int(we))
                ...
                has_next = (i + 1) < len(chunk_candidates)
                if _is_low_memory_error(e) and has_next:
                    next_len = chunk_candidates[i + 1]
                    print(
                        "[WARN] Overlap-chunk load failed by memory pressure.",
                        f"retry decode={next_len} (prev={cand_len})",
                    )
                    continue
                raise
        if last_err is not None:
            raise last_err
        raise RuntimeError("Failed to load propagation window.")
    ...
    def _consume_window(window_start: int, window_end: int, start_idx: int, reverse: bool) -> None:
        nonlocal last_frame_idx
        if state.inference_session is None:
            return
        ws = int(window_start)
        we = int(window_end)
        st = int(np.clip(int(start_idx), ws, max(ws, we - 1)))
        if bool(reverse):
            max_track = max(0, int(st) - int(ws))
        else:
            max_track = max(0, int(we) - int(st) - 1)

        for out in _iter_outputs(start_frame_idx=int(st), max_track=int(max_track), reverse=bool(reverse)):
            frame_idx = _session_to_global_frame_idx(state, int(out.frame_idx))
            if frame_idx < ws or frame_idx >= we:
                continue
    ...
    def _commit_range(commit_start: int, commit_end: int) -> Iterator[Tuple[AppState, str, Any, Image.Image]]:
        nonlocal last_frame_idx
        cs = int(commit_start)
        ce = int(commit_end)
        if ce <= cs:
            return
        for frame_idx in range(cs, ce):
            committed_frames.add(int(frame_idx))
            frame_masks = state.masks_by_frame.get(int(frame_idx), {})
            if frame_masks:
                masked_frames.add(int(frame_idx))
            else:
                masked_frames.discard(int(frame_idx))
            last_frame_idx = int(frame_idx)
            covered = len(committed_frames)
    ...
    first_forward_commit_range: Optional[Tuple[int, int]] = None
    first_seeded_forward_start: Optional[int] = None

    with torch.no_grad():
        # Forward pass: process [anchor_chunk_start, end) with overlapped windows.
        carry_seed_boxes: Dict[int, Tuple[int, int, int, int]] = {}
        base_start = int(anchor_chunk_start)
        while base_start < int(total):
            window_start = int(base_start)
            window_end_target = min(int(total), int(base_start) + int(decode_window))
            loaded_s, loaded_e = _load_window_session(int(window_start), int(window_end_target))
            if loaded_e <= loaded_s:
                break

            commit_start = int(base_start)
            commit_end = min(int(total), int(commit_start) + int(chunk_size), int(loaded_e))
            if commit_end <= commit_start:
                commit_end = int(loaded_e)

            local_prompt_frames = [f for f in prompt_frame_set if int(loaded_s) <= int(f) < int(loaded_e)]
            has_local_prompt = len(local_prompt_frames) > 0

            if has_local_prompt:
                start_fwd = int(min(local_prompt_frames))
                _consume_window(int(loaded_s), int(loaded_e), int(start_fwd), reverse=False)
                _consume_window(int(loaded_s), int(loaded_e), int(start_fwd), reverse=True)
            elif carry_seed_boxes:
                _inject_seed_boxes(
                    state,
                    processor,
                    frame_idx=int(commit_start),
                ...
                _consume_window(int(loaded_s), int(loaded_e), int(commit_start), reverse=False)

            if has_local_prompt or carry_seed_boxes:
                if first_seeded_forward_start is None:
                    first_seeded_forward_start = int(commit_start)
            else:
                # Keep moving until we reach a seeded window. Do not commit empty ranges
                # before the first seed, so backward pass can still fill them.
                base_start = int(max(int(base_start) + 1, int(loaded_e)))
                continue

            if first_forward_commit_range is None:
                first_forward_commit_range = (int(commit_start), int(commit_end))

            for item in _commit_range(int(commit_start), int(commit_end)):
                yield item

            overlap_start = int(commit_end)
            overlap_end = int(loaded_e)
            carry_seed_boxes = {}
            if overlap_end > overlap_start:
                carry_seed_boxes = _seed_boxes_from_chunk_scan(
                    state,
                    chunk_start=int(overlap_start),
                    chunk_end=int(overlap_end),
                    h=int(h),
                    w=int(w),
                    prefer_end=False,
                )
                # Keep only committed masks in memory. Overlap will be recomputed in next chunk.
                for fidx in range(int(overlap_start), int(overlap_end)):
                    state.masks_by_frame.pop(int(fidx), None)
                    state.composited_frames.pop(int(fidx), None)
            if (not carry_seed_boxes) and commit_end > commit_start:
                carry_seed_boxes = _seed_boxes_from_chunk_scan(
                    state,
                    chunk_start=int(commit_start),
                    chunk_end=int(commit_end),
                    h=int(h),
                    w=int(w),
                    prefer_end=True,
                )

            if commit_end <= base_start:
                base_start = int(loaded_e)
            else:
                base_start = int(commit_end)

        # Backward pass: process [0, backward_anchor_start) with overlapped windows.
        backward_anchor_start = (
            int(first_seeded_forward_start)
            if first_seeded_forward_start is not None
            else int(anchor_chunk_start)
        )
        if int(backward_anchor_start) > 0:
            carry_seed_boxes_back: Dict[int, Tuple[int, int, int, int]] = {}
            if first_forward_commit_range is not None:
                fs, fe = first_forward_commit_range
                if int(fe) > int(fs):
            ...
            base_end = int(backward_anchor_start)
            while base_end > 0:
                commit_end = int(base_end)
                commit_start = max(0, int(commit_end) - int(chunk_size))
                window_start = max(0, int(commit_end) - int(decode_window))
                loaded_s, loaded_e = _load_window_session(int(window_start), int(commit_end))
                if loaded_e <= loaded_s:
                    break

                commit_start = max(int(commit_start), int(loaded_s))
                commit_end = min(int(commit_end), int(loaded_e))
                if commit_end <= commit_start:
                    next_end = int(loaded_s)
                    if next_end >= int(base_end):
                        break
                    base_end = int(next_end)
                    continue
                ...
                if has_local_prompt:
                    ...
                    _consume_window(int(loaded_s), int(loaded_e), int(start_rev), reverse=True)
                    _consume_window(int(loaded_s), int(loaded_e), int(start_rev), reverse=False)
                elif carry_seed_boxes_back:
                    seed_frame = int(commit_end - 1)
                    _inject_seed_boxes(
                        state,
                        processor,
                    ...
                    _consume_window(int(loaded_s), int(loaded_e), int(seed_frame), reverse=True)

                for item in _commit_range(int(commit_start), int(commit_end)):
                    yield item

                carry_seed_boxes_back = {}
                if int(commit_start) > int(loaded_s):
                    carry_seed_boxes_back = _seed_boxes_from_chunk_scan(
                        state,
                        chunk_start=int(loaded_s),
                        chunk_end=int(commit_start),
                        h=int(h),
                        w=int(w),
                        prefer_end=True,
                    )
                    # Keep only committed masks in memory. Overlap will be recomputed in next chunk.
                    for fidx in range(int(loaded_s), int(commit_start)):
                        state.masks_by_frame.pop(int(fidx), None)
                        state.composited_frames.pop(int(fidx), None)
                if (not carry_seed_boxes_back) and int(commit_end) > int(commit_start):
                    carry_seed_boxes_back = _seed_boxes_from_chunk_scan(
                        state,
                        chunk_start=int(commit_start),
                        chunk_end=int(commit_end),
                ...
                if commit_start >= base_end:
                    next_end = int(loaded_s)
                    if next_end >= int(base_end):
                        break
                    base_end = int(next_end)
                else:
                    base_end = int(commit_start)

    # Final safety net for any residual uncommitted frames.
    if len(committed_frames) < total:
        for frame_idx in range(int(total)):
            if frame_idx in committed_frames:
                continue
            committed_frames.add(int(frame_idx))
            frame_masks = state.masks_by_frame.get(int(frame_idx), {})
            if frame_masks:
    ...
    state.current_frame_idx = int(np.clip(last_frame_idx, 0, max(0, state.num_frames - 1)))
    done_msg = (
        f"Propagation done. masked={masked}/{total}, visited={covered}/{total} "
...
def propagate_masks(state: AppState) -> Iterator[Tuple[AppState, str, Any, Image.Image]]:
    ...
    # Primary path: overlap-chunk propagation with bounded decode memory.
    try:
        ...
        if _JOB_USE_TORCH_COMPILE and _is_torch_compile_runtime_failure(e):
            ...
            for item in _propagate_masks_full_session_legacy(state, model_retry, processor_retry, prompt_frames):
                yield item
            return
        print(f"[WARN] Overlap-chunk propagation failed; falling back to legacy chunk mode: {e}")
...
def _iter_render_source_bgr_frames(
    state: AppState,
    width: int,
    height: int,
) -> Iterator[np.ndarray]:
    target = max(0, int(getattr(state, "num_frames", 0)))
    if target <= 0:
        return
            ...
            try:
                while idx < target:
                    ...
                    if ret and frame_bgr is not None:
                        ...
                        break
                    idx += 1
                while idx < target and last_frame is not None:
                    yield last_frame.copy()
                    idx += 1
                if idx >= target:
                    return
    ...
    # Fallback path: decode via OnDemandVideoFrames/PIL.
...
def render_fg_black_video(
    ...
    render_update_stride = 180
    for idx, frame_bgr in enumerate(_iter_render_source_bgr_frames(state, w, h)):
        union = _union_mask_for_frame(state, idx, h, w)  # bool
        out_bgr = frame_bgr.copy()
        out_bgr[~union] = 0
        writer.write(out_bgr)

        if (idx + 1) % render_update_stride == 0 or (idx + 1) == state.num_frames:
            prog = 0.62 + 0.10 * ((idx + 1) / max(1, state.num_frames))
...
def render_gbbb_video(
    ...
    for idx, frame_bgr in enumerate(_iter_render_source_bgr_frames(state, w, h)):
        ...
        out_bgr = frame_bgr.copy()
        out_bgr[~union] = bg_bgr

        writer.write(out_bgr)

        if (idx + 1) % render_update_stride == 0 or (idx + 1) == state.num_frames:
            prog = 0.65 + 0.25 * ((idx + 1) / max(1, state.num_frames))
...
def finish_job(
    ...
    try:
        ...
        try:
            ...
            # If the destination is locked by another process, keep the
            # job-unique output filename and continue successfully.
            print("[WARN] output rename failed; keeping job-scoped filename:", e)
            out_path = composited_path
            rename_note = (
                f"Named output file is in use and could not be replaced. "
                f"Used job output file instead: {out_path}"
            )
